...
export default function KnittingSent() {
  ...
  // ---------- LOAD PO NUMBERS (WITH TOTAL UNITS) ----------
  ...
  // ---------- LOAD EDIT DATA ----------
  const loadKnittingData = async () => {
    try {
      const docRef = doc(db, "knittingSent", id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        await updateDoc(doc(db, "knittingSent", id), data);
        toast.success("Knitting Updated Successfully!");
      } else {
        const docRef = await addDoc(collection(db, "knittingSent"), data);
        console.log("✅ Document written with ID:", docRef.id);
        toast.success("Knitting Sent Successfully!");
      }

      navigate("/knitting/sent-list");
    ...
    <div className="container-fluid p-4">
      ...
      <div className="text-center py-4 border-bottom mb-4">
        <h2 className="fw-bold text-uppercase mb-0" style={{ letterSpacing: "2px", fontSize: "2rem" }}>
          {id ? "EDIT" : "ADD"} KNITTING SENT
        </h2>
      </div>

      <form onSubmit={handleSubmit}>
        {/* TOP SECTION - PO#, ITEM, QUANTITY, ITEM QTY */}
        ...
        <div className="d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3 mb-4">
          ...
          <div className="d-flex gap-2">
            <Button type="button" variant="outline-secondary" onClick={() => navigate("/knitting/sent-list")}>
              Cancel
            </Button>
