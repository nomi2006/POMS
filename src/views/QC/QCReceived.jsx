...
export default function QCReceived() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const querySnapshot = await getDocs(collection(db, "qcSent"));
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteDoc(doc(db, "qcSent", id));
    ...
    <div className="container-fluid p-4">
      <Card className="shadow-sm border-0">
        <Card.Header className="bg-white border-0 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 py-3">
          <h4 className="fw-bold mb-0">QC Received</h4>
              ...
              <Table hover className="mb-0 align-middle" style={{ fontSize: "0.9rem" }}>
                <thead className="table-light">
                  <tr>
                    ...
                    <th className="text-center">OFF-LINE</th>
                  ...
                  {filtered.length > 0 ? (
                        ...
                        <td className="text-center">
                          <div className="d-flex gap-1 justify-content-center">
                            <Button
                              ...
                              onClick={() => navigate(`/qc/edit/${item.id}`)}
                            >
                              Edit
                            </Button>
                            <Button
                    ...
                    <tr>
                      <td colSpan="6" className="text-center py-4 text-muted">
                        No records found.{" "}
                        <Button variant="link" onClick={() => navigate("/qc/sent")}>
                          Create one now
                        </Button>
                      </td>
                    </tr>
