...
export default function QCSent() {
  ...
  const handleSubmit = async (e) => {
    ...
    const hasValidItem = tableItems.some(item =>
      item.style || item.color || item.inline || item.offline
    );
    ...
    try {
      ...
      if (id) {
        await updateDoc(doc(db, "qcSent", id), data);
        toast.success("QC Updated Successfully!");
      } else {
        const docRef = await addDoc(collection(db, "qcSent"), data);
        console.log("✅ Document written with ID:", docRef.id);
        toast.success("QC Sent Successfully!");
      }

      navigate("/qc/received");
      ...
      <form onSubmit={handleSubmit}>
              ...
              <Table borderless className="mb-0" style={{ fontSize: "0.85rem" }}>
                <thead>
                  <tr className="border-top border-bottom" style={{ backgroundColor: "#d9dcdf" }}>
                    ...
                    <th className="px-2 py-2 text-center fw-bold align-middle" style={{ width: "18%" }}>IN-LINE</th>
                    <th className="px-2 py-2 text-center fw-bold align-middle" style={{ width: "18%" }}>OFF-LINE</th>
                    ...
                    <tr key={index} className="border-bottom" style={{ backgroundColor: index % 2 === 0 ? "#fff" : "#f9f9f9" }}>
                      ...
                      <td className="px-2 py-2 align-middle">
                        <Form.Control
                          ...
                          type="number"
                          value={item.inline}
                          onChange={(e) => handleTableItemChange(index, "inline", e.target.value)}
                          placeholder="In-Line"
                      ...
                      <td className="px-2 py-2 align-middle">
                        <Form.Control
                          ...
                          type="number"
                          value={item.offline}
                          onChange={(e) => handleTableItemChange(index, "offline", e.target.value)}
                          placeholder="Off-Line"
                          className="text-center"
                          style={{ height: "36px", fontSize: "0.85rem" }}
                        />
                      </td>
        ...
        <div className="d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3 mb-4">
          ...
          <div className="d-flex gap-2">
            <Button type="button" variant="outline-secondary" onClick={() => navigate("/qc/received")}>
              Cancel
            </Button>
