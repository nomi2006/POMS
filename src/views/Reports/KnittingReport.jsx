...
export default function KnittingReport() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const snapshot = await getDocs(collection(db, "knittingSent"));
      const data = snapshot.docs.map((doc) => {
        ...
        return {
          ...
          qtyRequired: firstItem.qtyRequired || "-",
          yarnQty: firstItem.yarnQty || "-",
      ...
      toast.error("Failed to load knitting data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const calculateStats = (data) => {
    setStats({
      ...
      totalQty: data.reduce((sum, item) => sum + Number(item.qtyRequired || 0), 0)
    });
  };
  ...
  const exportPDF = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        doc.setFontSize(18);
        doc.text("Knitting Report", pageWidth / 2, 20, { align: "center" });
          ...
          <Row className="g-3">
            ...
            <Col md={3}>
              <Form.Group>
                ...
                <Form.Control type="text" name="search" value={filters.search} onChange={handleFilterChange} placeholder="Search by PO#, Style, Color..." />
              </Form.Group>
            </Col>
      ...
      <Row className="g-3 mb-4">
        <Col md={6}>
          <Card className="border-0 shadow-sm text-center bg-primary text-white">
            <Card.Body><h6 className="text-white-50">Total Records</h6><h2 className="fw-bold mb-0">{stats.totalRecords}</h2></Card.Body>
          </Card>
        </Col>
        <Col md={6}>
          <Card className="border-0 shadow-sm text-center bg-success text-white">
            <Card.Body><h6 className="text-white-50">Total Qty</h6><h2 className="fw-bold mb-0">{stats.totalQty}</h2></Card.Body>
          </Card>
        </Col>
      </Row>
                  ...
                  {filteredData.length > 0 ? (
                    filteredData.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td className="text-center">{item.qtyRequired}</td>
                        <td>{formatDate(item.date)}</td>
                        <td className="text-center">
                          <Button size="sm" variant="outline-primary" onClick={() => navigate(`/knitting/edit/${item.id}`)}>
