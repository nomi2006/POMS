...
export default function AccessoriesPurchaseReport() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const snapshot = await getDocs(collection(db, "accessoriesPurchases"));
      const data = snapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data()
      }));
  ...
  const calculateStats = (data) => {
    ...
    setStats({
      ...
      totalItems: data.reduce((sum, item) => sum + Number(item.totalItems || 0), 0),
  ...
  const resetFilters = () => {
    setFilters({ fromDate: "", toDate: "", search: "", client: "", stage: "" });
    setFilteredData(data);
    calculateStats(data);
  };
  ...
  const exportPDF = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        doc.setFontSize(18);
        doc.text("Accessories Purchase Report", pageWidth / 2, 20, { align: "center" });
        ...
        doc.text(`Total Items: ${stats.totalItems}`, 14, 50);
        ...
        autoTable(doc, {
          startY: 62,
          head: [["PO #", "Client", "Date", "Items", "Total", "Stage"]],
          body: tableData,
          theme: "striped",
        ...
        doc.save("accessories_purchase_report.pdf");
          ...
          <Row className="g-3">
            ...
            <Col md={3}>
              <Form.Group>
                ...
                <Form.Control type="text" name="search" value={filters.search} onChange={handleFilterChange} placeholder="Search by PO#, Client..." />
              </Form.Group>
            </Col>
            <Col md={3}>
              <Form.Group>
      ...
      <Row className="g-3 mb-4">
        ...
        <Col md={3}>
          <Card className="border-0 shadow-sm text-center bg-warning text-white">
            <Card.Body><h6 className="text-white-50">Total Items</h6><h2 className="fw-bold mb-0">{stats.totalItems}</h2></Card.Body>
          </Card>
        </Col>
        <Col md={3}>
                    ...
                    filteredData.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td className="text-center">
                          <Button size="sm" variant="outline-primary" onClick={() => navigate(`/accessories-purchase/view/${item.id}`)}>
