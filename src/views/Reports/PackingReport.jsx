...
export default function PackingReport() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const snapshot = await getDocs(collection(db, "packingSent"));
      const data = snapshot.docs.map((doc) => {
        ...
        return {
          ...
          cartons: firstItem.cartons || "-",
          packingType: firstItem.packingType || "-",
  ...
  const calculateStats = (data) => {
    setStats({
      ...
      totalQuantity: data.reduce((sum, item) => sum + Number(item.quantity || 0), 0),
      totalCartons: data.reduce((sum, item) => sum + Number(item.cartons || 0), 0)
    });
  };
  ...
  const applyFilters = () => {
    ...
    if (filters.packingType) {
      filtered = filtered.filter((item) => item.packingType === filters.packingType);
    }
  ...
  const exportPDF = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        doc.setFontSize(18);
        doc.text("Packing Report", pageWidth / 2, 20, { align: "center" });
        ...
        doc.text(`Total Quantity: ${stats.totalQuantity}`, 14, 44);
        doc.text(`Total Cartons: ${stats.totalCartons}`, 14, 50);
        ...
        autoTable(doc, {
          startY: 56,
          head: [["PO #", "Style", "Color", "Qty", "Cartons", "Packing Type"]],
          body: tableData,
          theme: "striped",
      ...
      <Card className="shadow-sm border-0 mb-4">
        <Card.Body>
          <Row className="g-3">
            ...
            <Col md={2}>
              <Form.Group>
                <Form.Label className="fw-bold small text-muted">PO #</Form.Label>
                <Form.Select name="poNumber" value={filters.poNumber} onChange={handleFilterChange}>
                  <option value="">All POs</option>
                  {poList.map((po) => (
                    <option key={po.id} value={po.po}>{po.po}</option>
                  ))}
                </Form.Select>
              </Form.Group>
            </Col>
            <Col md={2}>
              <Form.Group>
                <Form.Label className="fw-bold small text-muted">Packing Type</Form.Label>
                <Form.Select name="packingType" value={filters.packingType} onChange={handleFilterChange}>
            ...
            <Col md={3}>
              <Form.Group>
                ...
                <Form.Control type="text" name="search" value={filters.search} onChange={handleFilterChange} placeholder="Search..." />
              </Form.Group>
            </Col>
            <Col md={2} className="d-flex align-items-end gap-2">
              <Button variant="primary" onClick={applyFilters} className="w-100">
                <i className="ph ph-funnel me-1" />Apply
              </Button>
              <Button variant="outline-secondary" onClick={resetFilters} className="w-100">
                <i className="ph ph-arrow-clockwise me-1" />Reset
              </Button>
            </Col>
          </Row>
        </Card.Body>
      ...
      <Row className="g-3 mb-4">
        <Col md={4}>
          <Card className="border-0 shadow-sm text-center bg-primary text-white">
            <Card.Body>
              <h6 className="text-white-50">Total Records</h6>
              <h2 className="fw-bold mb-0">{stats.totalRecords}</h2>
            </Card.Body>
          </Card>
        </Col>
        <Col md={4}>
          <Card className="border-0 shadow-sm text-center bg-success text-white">
            <Card.Body>
              <h6 className="text-white-50">Total Qty</h6>
              <h2 className="fw-bold mb-0">{stats.totalQuantity}</h2>
            </Card.Body>
          </Card>
        </Col>
        <Col md={4}>
          <Card className="border-0 shadow-sm text-center bg-warning text-white">
            <Card.Body>
              <h6 className="text-white-50">Total Cartons</h6>
              <h2 className="fw-bold mb-0">{stats.totalCartons}</h2>
            </Card.Body>
          </Card>
        </Col>
      </Row>
      ...
      <Card className="shadow-sm border-0">
        <Card.Header className="bg-white d-flex justify-content-between align-items-center py-3">
          <h6 className="fw-bold mb-0">
            <i className="ph ph-list me-2 text-primary" />Records ({filteredData.length})
          </h6>
          <Button variant="success" onClick={exportPDF} size="sm">
            <i className="ph ph-file-pdf me-1" />Export PDF
          </Button>
        </Card.Header>
                  ...
                  {filteredData.length > 0 ? (
                    filteredData.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td>{item.style || "-"}</td>
                        <td>
                          <span className="d-inline-block me-1" style={{ width: "12px", height: "12px", backgroundColor: item.color || "#ddd", borderRadius: "50%" }} />
                          {item.colorName || item.color || "-"}
                        </td>
                        <td className="text-center">{item.quantity || 0}</td>
                        <td className="text-center">{item.cartons || 0}</td>
                        <td>{item.packingType || "-"}</td>
                        <td className="text-center">
                          <Button size="sm" variant="outline-primary" onClick={() => navigate(`/packing/edit/${item.id}`)}>
