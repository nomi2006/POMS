...
export default function QCReport() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const snapshot = await getDocs(collection(db, "qcSent"));
      const data = snapshot.docs.map((doc) => {
        ...
        return {
          ...
          inline: firstItem.inline || "-",
          offline: firstItem.offline || "-",
  ...
  const calculateStats = (data) => {
    setStats({
      ...
      totalInline: data.reduce((sum, item) => sum + Number(item.inline || 0), 0),
      totalOffline: data.reduce((sum, item) => sum + Number(item.offline || 0), 0)
    });
  };
  ...
  const resetFilters = () => {
    setFilters({ fromDate: "", toDate: "", search: "", poNumber: "" });
    setFilteredData(data);
    calculateStats(data);
  };
  ...
  const exportPDF = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        doc.setFontSize(18);
        doc.text("QC Report", pageWidth / 2, 20, { align: "center" });
        ...
        doc.text(`Total In-Line: ${stats.totalInline}`, 14, 44);
        doc.text(`Total Off-Line: ${stats.totalOffline}`, 14, 50);
        ...
        autoTable(doc, {
          startY: 56,
          head: [["PO #", "Style", "Color", "In-Line", "Off-Line"]],
          body: tableData,
          theme: "striped",
  ...
  return (
    <div className="container-fluid p-4">
      ...
      <Row className="g-3 mb-4">
        <Col md={4}><Card className="border-0 shadow-sm text-center bg-primary text-white"><Card.Body><h6 className="text-white-50">Total Records</h6><h2 className="fw-bold mb-0">{stats.totalRecords}</h2></Card.Body></Card></Col>
        <Col md={4}><Card className="border-0 shadow-sm text-center bg-success text-white"><Card.Body><h6 className="text-white-50">Total In-Line</h6><h2 className="fw-bold mb-0">{stats.totalInline}</h2></Card.Body></Card></Col>
        <Col md={4}><Card className="border-0 shadow-sm text-center bg-warning text-white"><Card.Body><h6 className="text-white-50">Total Off-Line</h6><h2 className="fw-bold mb-0">{stats.totalOffline}</h2></Card.Body></Card></Col>
      </Row>
          ...
          {loading ? (
            ...
            <div className="table-responsive">
              <Table hover className="mb-0 align-middle" style={{ fontSize: "0.9rem" }}>
                <thead className="table-light"><tr><th>PO #</th><th>Style</th><th>Color</th><th className="text-center">In-Line</th><th className="text-center">Off-Line</th><th className="text-center">Action</th></tr></thead>
                <tbody>
                  {filteredData.length > 0 ? (
                    filteredData.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td className="text-center">{item.inline}</td>
                        <td className="text-center">{item.offline}</td>
                        <td className="text-center">
                          <Button size="sm" variant="outline-primary" onClick={() => navigate(`/qc/edit/${item.id}`)}><i className="ph ph-eye" /></Button>
                        </td>
                      </tr>
                    ))
                  ) : (
