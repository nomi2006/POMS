...
export default function DyingReport() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const snapshot = await getDocs(collection(db, "dyingSent"));
      const data = snapshot.docs.map((doc) => {
      ...
      toast.error("Failed to load dying data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportPDF = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        doc.setFontSize(18);
        doc.text("Dying Report", pageWidth / 2, 20, { align: "center" });
      ...
      <Card className="shadow-sm border-0 mb-4">
        <Card.Body>
          <Row className="g-3">
            ...
            <Col md={3}>
              <Form.Group><Form.Label className="fw-bold small text-muted">Search</Form.Label><Form.Control type="text" name="search" value={filters.search} onChange={handleFilterChange} placeholder="Search by PO#, Style, Color..." /></Form.Group>
            </Col>
          ...
          {loading ? (
            ...
            <div className="table-responsive">
              <Table hover className="mb-0 align-middle" style={{ fontSize: "0.9rem" }}>
                <thead className="table-light"><tr><th>PO #</th><th>Style</th><th>Color</th><th className="text-center">Qty</th><th>Date</th><th className="text-center">Action</th></tr></thead>
                <tbody>
                    ...
                    filteredData.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td className="text-center">
                          <Button size="sm" variant="outline-primary" onClick={() => navigate(`/dying/edit/${item.id}`)}><i className="ph ph-eye" /></Button>
                        </td>
                      </tr>
                    ))
                  ) : (
