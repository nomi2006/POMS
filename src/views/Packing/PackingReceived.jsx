...
export default function PackingReceived() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const querySnapshot = await getDocs(collection(db, "packingSent"));
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteDoc(doc(db, "packingSent", id));
  ...
  const handleSearch = (e) => {
    ...
    const filteredData = data.filter((item) =>
      ...
      item.packingType?.toLowerCase().includes(value)
    );
    ...
    <div className="container-fluid p-4">
      <Card className="shadow-sm border-0">
        <Card.Header className="bg-white border-0 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 py-3">
          <h4 className="fw-bold mb-0">Packing Received</h4>
                  ...
                  {filtered.length > 0 ? (
                    filtered.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td className="text-center">{item.cartons || "-"}</td>
                        <td className="text-center">{item.packingType || "-"}</td>
                        ...
                        <td className="text-center">
                          <div className="d-flex gap-1 justify-content-center">
                            <Button
                              ...
                              onClick={() => navigate(`/packing/edit/${item.id}`)}
                            >
                              Edit
                            </Button>
                            <Button
                    ...
                    <tr>
                      <td colSpan="8" className="text-center py-4 text-muted">
                        No records found.{" "}
                        <Button variant="link" onClick={() => navigate("/packing/sent")}>
                          Create one now
                        </Button>
                      </td>
                    </tr>
