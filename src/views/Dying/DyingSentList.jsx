...
export default function DyingSentList() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const querySnapshot = await getDocs(collection(db, "dyingSent"));
      const data = querySnapshot.docs.map((doc) => {
        ...
        const sentQty = Number(firstItem.qtyRequired) || 0;
        const rcvdQty = Number(firstItem.rcvdQty) || 0;
        const balanceQty = sentQty - rcvdQty;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteDoc(doc(db, "dyingSent", id));
    ...
    <div className="container-fluid p-4">
      <Card className="shadow-sm border-0">
        <Card.Header className="bg-white border-0 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 py-3">
          <h4 className="fw-bold mb-0">Dying List</h4>
          <div className="d-flex gap-2 w-100 w-md-auto">
            <Form.Control
              type="text"
              placeholder="Search by PO, Item, Style, Color..."
              value={search}
              ...
              <Table hover className="mb-0 align-middle" style={{ fontSize: "0.9rem" }}>
                <thead className="table-light">
                  <tr>
                    ...
                    <th className="text-center">Sent QTY</th>
                  ...
                  {filtered.length > 0 ? (
                    filtered.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td className="text-center">{item.sentQty || 0}</td>
                        <td className="text-center">{item.rcvdQty || 0}</td>
                        <td className="text-center fw-bold">{item.balanceQty || 0}</td>
                        <td className="text-center">
                          <div className="d-flex gap-1 justify-content-center">
                            <Button
                              size="sm"
                              variant="outline-warning"
                              onClick={() => navigate(`/dying/edit/${item.id}`)}
                            >
                              Edit
                            </Button>
                            <Button
                    ...
                    <tr>
                      <td colSpan="8" className="text-center py-4 text-muted">
                        No records found.{" "}
                        <Button variant="link" onClick={() => navigate("/dying/sent")}>
                          Create one now
                        </Button>
                      </td>
                    </tr>
